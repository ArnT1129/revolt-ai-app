...
import { UploadCloud, File, X, CheckCircle, AlertCircle, FileText, Eye } from 'lucide-react';
...
export default function FileUploader() {
  ...
  const generateBatteryFromAnalysis = (result: any, filename: string): Battery => {
    const batteryId = filename.replace(/\.(csv|xlsx|xls)$/i, '').toUpperCase();
    
    // Calculate SoH based on capacity degradation (simplified)
    const avgCapacity = result.data.reduce((sum: number, row: any) => sum + (row.capacity_mAh || 0), 0) / result.data.length;
    const maxCapacity = Math.max(...result.data.map((row: any) => row.capacity_mAh || 0));
    const soh = Math.min(99.8, Math.max(70, (avgCapacity / maxCapacity) * 100));
    ...
    const sohHistory = Array.from({ length: Math.min(20, result.metadata.totalCycles) }, (_, i) => ({
      cycle: i * Math.floor(result.metadata.totalCycles / 20),
    ...
    return {
      ...
      rul,
      cycles: result.metadata.totalCycles,
      chemistry: result.metadata.chemistry as 'LFP' | 'NMC',
