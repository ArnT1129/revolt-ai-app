...
export default function BatteryTable() {
  ...
  const loadBatteries = async () => {
    ...
    try {
      setLoading(true);
      const userBatteries = await batteryService.getUserBatteries(user.id);
  ...
  const handleDeleteBattery = async (batteryId: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Battery has been removed from the system`,
      });
    } catch (error) {
  ...
  const handleAddManualBattery = async (newBattery: Omit<Battery, 'id' | 'uploadDate'>) => {
    if (!user) return;
    ...
    try {
      ...
      const createdBattery = await batteryService.createBattery(batteryData);
      setBatteries(prev => [...prev, createdBattery]);
      
      toast({
        title: "Battery Added",
        description: `Battery ${createdBattery.id} has been added to the system`,
      });
    } catch (error) {
  ...
  const handleSaveBattery = async (updatedBattery: Battery) => {
    try {
      const saved = await batteryService.updateBattery(updatedBattery.id, updatedBattery);
      setBatteries(prev => 
        prev.map(battery => 
          battery.id === updatedBattery.id ? saved : battery
        )
      );
      setSelectedBattery(saved);
      
      toast({
        title: "Battery Updated",
        description: "Battery information has been saved",
      });
    } catch (error) {
    ...
    return (
      <Card>
        <CardContent className="p-8 text-center">
          <div className="text-white">Loading your batteries...</div>
        </CardContent>
      </Card>
    );
  }
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-white">My Battery Fleet</CardTitle>
        ...
        <CardContent className="p-0">
          {batteries.length === 0 ? (
            <div className="p-8 text-center">
              <p className="text-slate-400 mb-4">No batteries found. Upload data or add a battery manually to get started.</p>
